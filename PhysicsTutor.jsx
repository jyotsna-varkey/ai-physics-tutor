...
import { Send, BookOpen, Target, Award, RotateCcw, Clock, Zap, CheckCircle, XCircle } from 'lucide-react';
...
const DIAGNOSTIC_MCQs = {
  universal_law: {
    ...
    correctAnswer: 'b',
    explanation: "Force ∝ 1/d². If d doubles, force becomes 1/4."
  },
  free_fall: {
...
const CONCEPT_MCQs = {
  ...
  acceleration_gravity: [
    ...
    },
    {
      ...
      correct: 0,
      explanation: "g ∝ 1/R². If R doubles, g becomes g/4."
    }
  ],
  mass_weight: [
    {
    ...
    },
    {
      ...
      correct: 0,
      explanation: "Moon's gravity ≈ (1/6) of Earth's."
    },
    {
...
const RAPID_FIRE = [
  ...
  { q: "Earth mass 4×, radius 2×, g becomes:", opts: ["Same", "2g", "g/2", "4g"], c: 0, t: 30 }
];
...
const IntegratedPhysicsTutor = () => {
  ...
  const startLearningConcept = (index, queueOverride) => {
    ...
    addMessage('assistant',
      `## 🎯 ${concept.name}\n\n` +
      `**Key ideas:**\n` +
      concept.keyIdeas.map(idea => `• ${idea}`).join('\n')
    );
  ...
  const evaluateResponse = async (conceptId, userMsg) => {
    ...
    try {
      ...
      // Parse result
      const grade = data.result.charAt(0); // A, B, or C
      return { grade, explanation: data.result.substring(4) };
    } catch (e) {
  ...
  const getSocraticResponse = async (conceptId, userMsg, count, evaluation) => {
    ...
    try {
      ...
      console.error('Socratic error:', e);
      return count < 5 
...
export default IntegratedPhysicsTutor;
